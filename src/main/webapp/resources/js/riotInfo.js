...
async function leagueInfo(encID) {
  ...
  try {
    ...
    if (data == "") {
      $("#riotSummonerTbl3").css("opacity", "0%");
      let name = $('SN').val();
...
function onJsp(leagueInfo) {
  ...
  $(".riotSummonerTbl").css("opacity", "100%");
...
async function matchInfo(matchId, encpuuid) {
  ...
  for (let i = 0; i < matchId.length; i++) {
    ...
    // 검색한 사람 닉네임.
    let summonerName = await data.info.participants[indexForSearcherPuuid].summonerName;
